...
namespace ofxCv {
	...
	struct CompareContourArea
	{
		...
		// Sort contour indices into decreasing order, based on a std::vector of
		// contour areas.  Later, we will use these indices to order the
		// contours (which are stored in a separate std::vector).
	...
	void ContourFinder::findContours(Mat img) {
		...
		std::vector<vector<cv::Point> > allContours;
		...
		if (allIndices.size() > 1 && sortBySize) {
			// Sort contour indices, based on a separate std::vector of areas.
	...
	const std::vector<vector<cv::Point> >& ContourFinder::getContours() const {
		return contours;
	}
